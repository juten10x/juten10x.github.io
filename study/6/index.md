...
### juten10xについて
juten10x という HN は、結構気に入ってます。特に、誰とも被っていないという点。試しに Google 検索してみても、私に関係するものしか出てきません。 Microsoft Edge はこの限りではありませんが、リザルトの一番上にはこのサイトが表示されます。また、 juten10x と完全に一致する検索結果としては私に関するもののみとなります。実際、これまでこの HN で登録できなかったサービスはありませんでした<sup><a id="_note2" href="#note2">[注 2]</a></sup>。また、数字が入っているというのも嬉しいところ。アルファベットだけでなく数字も含まれていることでメリハリがつきます。また、この HN の読み方は特に決めているわけではありませんが、私は「じゅてんてんくす」と読んでいます。単に「じゅてん」でも、「 10 」でも良いですし、読み方はお任せしたいと思います。

### 脚注
#### 注釈
1.<a id="note1" href="#_note1">^ </a>Twitter は大文字でも ID として使うことが可能です

2.<a id="note2" href="#_note2">^ </a>このサイトの[トップページ](https://juten10x.github.io)をご覧になればわかる通り、 Wikipedia を除いては Codeforces では Juten10x 、 Reddit では _juten10x と、異なるものになっています。また、実は cluster の ID は 10x としています。これらには理由があって、 Codeforces は競プロerの方々はご存知かと思いますが、年末年始にのみユーザ名の変更が可能です。 Reddit ではもともと、 Juten10x という ID でアカウントを持っていましたが、 Reddit ではアカウントを作成した日から（たぶん）30日以内しか ID は変更できません。なので変えようと思っても変えられないわけですが、どうしても大文字が嫌だった私はアカウントを削除し（実際に Juten10x という ID のアカウントのプロフィールへアクセスしようとすると、このアカウントは削除された旨の記述が表示されます）、新たに _juten10x を ID としたアカウントを作成しました。また、 cluster については、しばらく cluster を使っていない間に、ログインに使っていた Twitter アカウントへのアクセスを失っていたために仕方なく新たにアカウントを作成したからです。また、以前使っていた際にすでに ID を juten10x にしていたため、新たに作成したアカウントでは他のものにせざるを得なくなりました。実際、今も juten10x という ID の cluster アカウントは存在しています。
